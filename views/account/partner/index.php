...
<div class="w-100 wrapper_form-balance">
    <?php if($user->id <> $identity->id): ?>
        <p class="text-left"><?=Yii::t('account', 'Personally invited by user')?>: <b><?=$user->username?></b></p>
    ...
    <table class="table table-striped">
        ...
        <?php foreach($users as $referral): ?>
            ...
            <?php $activations = $referral->activeActivations; ?>
            <tr>
                ...
                <?php for($n = 1; $n <= 6; $n++): ?>
                    <th>
                        <?php if($referral->status == User::STATUS_ACTIVE): ?>
                            ...
                            <?php if($not): ?>
                                <?php if($n == 1 && $identity->balance - $identity->accumulation >= Yii::$app->settings->get('system', 'activationAmount')): ?>
                                    <a href="/pm/activation/<?=$referral->id?>" class="btn btn-success"><?=Yii::t('account', 'Activate from own balance')?></a>
                                <?php else: ?>
